...
def main(k=1, c=1, NumDomain=2000, NumResamples=100): # Main Code
    ...
    dde.saveplot(losshistory, train_state, issave=True, isplot=False, 
                 loss_fname=f"RAD_RAND_uxt_k{k}c{c}_N{NumDomain}_L{NumResamples}_loss_info.dat", 
                 train_fname=f"RAD_RAND_uxt_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalpoints.dat", 
                 test_fname=f"RAD_RAND_uxt_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalypred.dat",
                 output_dir="../results/loss_points_prediction")
...
if __name__ == "__main__":
    ...
    error_hist, error_final, time_taken = main(c=c, k=k, NumDomain=NumDomain,NumResamples=NumResamples) # Run main, record error history and final accuracy.
    ...
    error_hist_fname = f"RAD_RAND_uxt_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_hist.txt"
    error_final_fname = f"RAD_RAND_uxt_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"RAD_RAND_uxt_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
