"""Run PINN to solve Burger's Equation using adaptive resampling (RAD) based on loss and then gradient/curvature information. 
...
def main(k=1, c=1, NumDomain=2000, NumResamples=100, method="Random"): # Main Code
    ...
    dde.saveplot(losshistory, train_state, issave=True, isplot=False, 
                 loss_fname=f"RAD_loss-uxut4_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_loss_info.dat", 
                 train_fname=f"RAD_loss-uxut4_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalpoints.dat", 
                 test_fname=f"RAD_loss-uxut4_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalypred.dat",
...
if __name__ == "__main__":
    ...
    error_hist_fname = f"RAD_loss-uxut4_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_hist.txt"
    error_final_fname = f"RAD_loss-uxut4_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"RAD_loss-uxut4_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
