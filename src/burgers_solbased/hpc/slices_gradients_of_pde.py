""" slices_gradients_of_pde.py Inserted two functions for d(pde)/dx and d(pde)/dt so can calculate how those look throughout solution process for Burger's Equation, and plots for me to see. Will have to do with a smaller number of points.


Usage:
    slices_gradients_of_pde.py [--k=<hyp_k>] [--c=<hyp_c>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>]
    slices_gradients_of_pde.py -h | --help
Options:
...
if __name__ == "__main__":
    ...
    error_final_fname = f"slices_pde_gradients_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"slices_pde_gradients_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
