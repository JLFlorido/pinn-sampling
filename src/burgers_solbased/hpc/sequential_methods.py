...
sequential_methods.py. Uses two sources of information sequentially, half the loops for each.
December: Updated to still work if NumResamples is odd by making first loop one greater.
December: First loop doesn't use specified k and c, uses k=1 and c=1. Watch out.
Need to add pdedxt, pdedx and pdedt as methods.

Previously dual_sequential.py

Usage:
    sequential_methods.py [--k=<hyp_k>] [--c=<hyp_c>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>] [--INP2=<input2>]
    sequential_methods.py -h | --help
Options:
...
def main(k=1, c=1, NumDomain=2000, NumResamples=100, method="Random", depth=3, input1="residual", input2="uxt"): # Main Code
    ...
    # Loop using input 1
    for i in range(Num1): # Resampling loop begins. 100 is default, first run took ~4 hours...
        ...
        if input1 == "residual":
            ...
            Y2 = np.abs(model.predict(X, operator=dudt)).astype(np.float64)
            Y = np.sqrt((Y1**2)+(Y2**2))
        err_eq = np.power(Y, 1) / np.power(Y, 1).mean() + 1
    ...
    # Loop using input 2
    for i in range(Num2): # Resampling loop begins. 100 is default, first run took ~4 hours...
        X = geomtime.random_points(100000)
...
if __name__ == "__main__":
    ...
    error_final_fname = f"sequential_{input1}_{input2}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"sequential_{input1}_{input2}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
