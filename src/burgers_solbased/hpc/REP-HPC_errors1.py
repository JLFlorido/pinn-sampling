""" REP-HPC_errors1.py 1 because it uses the original Burgers.npz

Usage:
    REP-HPC_errors.py [--k=<hyp_k>] [--c=<hyp_c>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<Depth>]
...
def main(k=1, c=1, NumDomain=2000, NumResamples=100, method="Random", depth=3): # Main Code
    ...
    dde.saveplot(losshistory, train_state, issave=True, isplot=False, 
                 loss_fname=f"REP_errors1_res_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_loss_info.dat", 
                 train_fname=f"REP_errors1_res_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalpoints.dat", 
                 test_fname=f"REP_errors1_res_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalypred.dat",
...
if __name__ == "__main__":
    ...
    error_curves_fname = f"REP_errors1_res_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_curves.txt"
    error_final_fname = f"REP_errors1_res_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"REP_errors1_res_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
