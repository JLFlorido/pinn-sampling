...
slices.py. This code saves points, prediction, and new points based on prediction. At L=0 and L=50 (can modify). Saves data for visualisation through other code.

Usage:
    slices.py [--k=<hyp_k>] [--c=<hyp_c>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>]
    slices.py -h | --help
Options:
...
def main(k=1, c=1, NumDomain=2000, NumResamples=100, method="Random", depth=3, input1="residual"): # Main Code
    ...
    for i in range(NumResamples): # Resampling loop begins. 100 is default, first run took ~4 hours...
        ...
        if i in [0,49]:
            ...
            if input1 == "pdedxt":
                ...
                np.savetxt(f"pred_x_pdext_{i}.txt",X)
                np.savetxt(f"points_after_pdext_{i}.txt",X_selected)
                
                print("saved pred")
            elif input1 == "residual":
                np.savetxt(f"points_before_pde_{i}.txt", points_before)
                np.savetxt(f"pred_sol_pde_{i}.txt",Y)
                np.savetxt(f"pred_x_pde_{i}.txt",X)
                np.savetxt(f"points_after_pde_{i}.txt",X_selected)

                print("saved pred")
            elif input1 == "uxt":
                np.savetxt(f"points_before_uxt_{i}.txt", points_before)
                np.savetxt(f"pred_sol_uxt_{i}.txt",Y)
                np.savetxt(f"pred_x_uxt_{i}.txt",X)
                np.savetxt(f"points_after_uxt_{i}.txt",X_selected)

                print("saved pred")

    # End of taking the slices
    ...
    #              loss_fname=f"slices_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_loss_info.dat", 
    #              train_fname=f"slices_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalpoints.dat", 
    #              test_fname=f"slices_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_finalypred.dat",
...
if __name__ == "__main__":
    ...
    error_final_fname = f"slices_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"slices_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
