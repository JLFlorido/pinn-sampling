...
damping.py. Instead of a static c being chosen, the value of c decreases every loop from 1 to 0.

Usage:
    damping.py [--k=<hyp_k>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>]
    damping.py -h | --help
Options:
...
def main(k=1, NumDomain=2000, NumResamples=100, method="Random", depth=3, input1="residual"): # Main Code
    print(f"k equals {k}")
    ...
    #              loss_fname=f"damping_{input1}_D{depth}_{method}_k{k}_N{NumDomain}_L{NumResamples}_loss_info.dat", 
    #              train_fname=f"damping_{input1}_D{depth}_{method}_k{k}_N{NumDomain}_L{NumResamples}_finalpoints.dat", 
    #              test_fname=f"damping_{input1}_D{depth}_{method}_k{k}_N{NumDomain}_L{NumResamples}_finalypred.dat",
...
if __name__ == "__main__":
    ...
    error_final, time_taken = main(k=k, NumDomain=NumDomain,NumResamples=NumResamples,method=method, depth=depth, input1=input1) # Run main, record error history and final accuracy.
