""" pde_gradients_info_and_points.py Inserted two functions for d(pde)/dx and d(pde)/dt so can calculate how those look throughout solution process for Burger's Equation, and plots for me to see. Will have to do with a smaller number of points.


Usage:
    pde_gradients_info_and_points.py [--k=<hyp_k>] [--c=<hyp_c>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>]
    pde_gradients_info_and_points.py -h | --help
Options:
...
def main(k=1, c=1, NumDomain=2000, NumResamples=100, method="Random", depth=3, input1="residual"): # Main Code
    ...
    for i in range(NumResamples): # Resampling loop begins. 100 is default, first run took ~4 hours...
        ...
        if i in [0,19,39,59,79,99]:
            if input1 == "pdedx":
                ...
                np.savetxt(f"pred_dpde_dx_{i}.txt",dpde_dx_pred)
            elif input1 =="pdedt":
                dpde_dt_pred = model.predict(X_test, dpde_dt)
                np.savetxt(f"pred_dpde_dt_{i}.txt",dpde_dt_pred)
            elif input1 == "pdedxt":
                dpde_dxt_pred = model.predict(X_test, dpde_dxt)
                np.savetxt(f"pred_dpde_dxt_{i}.txt",dpde_dxt_pred)

            point_fname = f"points_{input1}_{i}.txt"
            np.savetxt(point_fname,X_selected)
    ...
    print("Done training, now saving final error and time taken")
...
if __name__ == "__main__":
    ...
    error_final_fname = f"pde_gradients_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"pde_gradients_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
