...
def gen_testdata(): # This function opens the ground truth solution. Need to change path directory for running in ARC.
    ...
    t, x, exact = data["t"], data["x"], data["exact"].T # If it's not "exact" its "usol"
    xx, tt = np.meshgrid(x, t)
    X = np.vstack((np.ravel(xx), np.ravel(tt))).T
    y = exact.flatten()[:, None]
    return xx, tt, exact
xx,tt,y_true = gen_testdata()


# Figure 0
plt.rcParams.update({'font.size': 14})
fig0, ax0 = plt.subplots(figsize=(8, 5))
scatter0 = ax0.contourf(tt,xx, y_true, cmap='rainbow', levels=20)
plt.colorbar(scatter0, ax=ax0, label='u(x,t)',ticks=np.arange(-1, 1.2, 0.2))
plt.subplots_adjust(left=0.14, bottom=0.12, right=0.97, top=0.97, wspace=None, hspace=None)
ax0.set_xlabel('t')
ax0.set_ylabel('x')
plt.tight_layout
plt.savefig('results/plots/eps files/Sample_Sol.eps', format='eps', dpi=1200)
plt.show()
# ---- ---- ----
# # Error
