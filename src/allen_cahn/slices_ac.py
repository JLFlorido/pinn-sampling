...
slices_ac.py Saving slice of resampling at L=0 and L=49

Usage:
    slices_ac.py [--D=<DiffCoeff>] [--k=<hyp_k>] [--c=<hyp_c>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>]
    slices_ac.py -h | --help
Options:
...
def main(diff=0.001, k=1, c=1, NumDomain=2000, NumResamples=100, method="Random", depth=3, input1="residual"): # Main Code
    ...
    # For saving more than one slice
    savenum = np.random.randint(1, 4)
...
# Taking the slices
        if i in [0,49]:
            ...
            if input1 == "pdedxt":
                np.savetxt(f"ac_points_before_pdext_{i}_{savenum}.txt", points_before)
                np.savetxt(f"ac_pred_sol_pdext_{i}_{savenum}.txt",Y)
                np.savetxt(f"ac_pred_x_pdext_{i}_{savenum}.txt",X)
                np.savetxt(f"ac_points_after_pdext_{i}_{savenum}.txt",X_selected)

                print("saved pred")
            elif input1 == "residual":
                np.savetxt(f"ac_points_before_pde_{method}_{i}_{savenum}.txt", points_before)
                np.savetxt(f"ac_pred_sol_pde_{method}_{i}_{savenum}.txt",Y)
                np.savetxt(f"ac_pred_x_pde_{method}_{i}_{savenum}.txt",X)
                np.savetxt(f"ac_points_after_pde_{method}_{i}_{savenum}.txt",X_selected)

                print("saved pred")
            elif input1 == "uxt":
                np.savetxt(f"ac_points_before_uxt_{i}_{savenum}.txt", points_before)
                np.savetxt(f"ac_pred_sol_uxt_{i}_{savenum}.txt",Y)
                np.savetxt(f"ac_pred_x_uxt_{i}_{savenum}.txt",X)
                np.savetxt(f"ac_points_after_uxt_{i}_{savenum}.txt",X_selected)
...
if __name__ == "__main__":
    ...
    error_final_fname = f"allencahn_slices_{diff}_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"allencahn_slices_{diff}_{input1}_D{depth}_{method}_k{k}c{c}_N{NumDomain}_L{NumResamples}_time_taken.txt"
