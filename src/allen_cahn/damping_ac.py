...
damping_ac.py Instead of a static c, the value of c decreases from 1 to 0 linearly.

Usage:
    main_ac.py [--D=<DiffCoeff>] [--k=<hyp_k>] [--N=<NumDomain>] [--L=<NumResamples> ] [--IM=<InitialMethod>] [--DEP=<depth>] [--INP1=<input1>]
    main_ac.py -h | --help
Options:
...
def main(diff=0.001, k=1, NumDomain=2000, NumResamples=100, method="Random", depth=3, input1="residual"): # Main Code
...
if __name__ == "__main__":
    ...
    error_final, time_taken = main(diff=diff, k=k, NumDomain=NumDomain,NumResamples=NumResamples,method=method, depth=depth, input1=input1) # Run main, record error history and final accuracy.
    ...
    error_final_fname = f"damping_linear_{diff}_{input1}_D{depth}_{method}_k{k}_N{NumDomain}_L{NumResamples}_error_final.txt"
    time_taken_fname = f"damping_linear_{diff}_{input1}_D{depth}_{method}_k{k}_N{NumDomain}_L{NumResamples}_time_taken.txt"
